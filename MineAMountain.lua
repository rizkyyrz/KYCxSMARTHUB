...
local farmCrystalSeekActive = false
...
local function trackingEnabled()
	return espActive or farmCrystalSeekActive
end
...
do
	local function install()
		...
		local safeFallParams = RaycastParams.new()
		safeFallParams.FilterType = Enum.RaycastFilterType.Exclude
		safeFallParams.IgnoreWater = false
		...
		local function safeFallStep()
			if tpState or flyActive then
				return
			end
			...
			if not character or not root or not humanoid or humanoid.Health <= 0 or humanoid.SeatPart then
				return
			end

			if root:FindFirstChild("UniverseGlidePosition") then
				return
			end
			...
			if current.Y >= -maxFallSpeed then
				return
			end

			safeFallParams.FilterDescendantsInstances = { character }

			local probeDistance = math.clamp(math.abs(current.Y) * 0.12, 6, 30)
			local hit = Workspace:Raycast(root.Position, Vector3.new(0, -probeDistance, 0), safeFallParams)
			local vertical = hit and -4 or -maxFallSpeed

			root.AssemblyLinearVelocity = Vector3.new(current.X, vertical, current.Z)
		end
		...
		function Move.setSafeFall(value)
			safeFallActive = value

			if value then
				if not safeFallConn then
					safeFallConn = RunService.Heartbeat:Connect(function()
						if not safeFallActive then
							return
						end

						local ok, err = pcall(safeFallStep)
						if not ok then
							reportError("safeFall", err)
						end
					end)
				end
			...
			if safeFallConn then
				safeFallConn:Disconnect()
				safeFallConn = nil
			end
		end

		function Move.setSafeFallSpeed(value)
			maxFallSpeed = math.clamp(tonumber(value) or 25, 8, 80)
		end
...
do
	local function install()
		...
		CrystalBox:AddLabel("Min Value hides/skips cheaper crystals and becomes the priority target for Money Auto Farm. Example: 100m makes Auto Farm chase crystals worth at least $100M. Empty shows everything", true)
...
do
	local function install()
		...
		MoveBox:AddToggle("NoFallDamage", {
			Text = "No Fall Damage",
			Default = false,
			Callback = Move.setSafeFall,
		})

		MoveBox:AddSlider("SafeFallSpeed", {
			Text = "Max Fall Speed",
			Default = 25,
			Min = 8,
			Max = 80,
			Rounding = 0,
			Compact = false,
			Callback = Move.setSafeFallSpeed,
		})
...
do
	local function install()
		...
		local DIG_SINK = 1.2
		local DIG_LIFT = 6
		local DIG_MIN_INTERVAL = 0.02
		local DIG_MAX_REQUESTS = 14
		local DIG_SLACK = 1.5
		local DIG_AIM = 0.999
		local EQUIP_STEP = 1
		local SELL_MARK = 0.5
		...
		local GRAB_GAP = 0.05
		local SAFETY_PLATFORM_SIZE = 18
		local SAFETY_DEFAULT_GAP = 8
		local SAFETY_RECOVER_MARGIN = 2.5
		local SAFETY_CRYSTAL_MARGIN = 0.75
		...
		-- Runtime dig tuning. Existing behaviour is preserved by the defaults:
		-- 2.5x rate, seven vertical hits and no radial spread.
		local digRate = 2.5
		local digDepth = DIG_BURST
		local digRadius = 0
		local digPatternCursor = 0

		-- Local safety floor plus emergency recovery. The floor gives the crystal
		-- and character some space below the active dig point, while recovery is
		-- still effective when Auto Farm has noclip enabled.
		local safetyEnabled = true
		local safetyGap = SAFETY_DEFAULT_GAP
		local safetyPlatform
		local lootSafetySpot
		...
		local function safetyFloorAt(focus)
			if not safetyEnabled or typeof(focus) ~= "Vector3" then
			...
			if not safetyPlatform or not safetyPlatform.Parent then
				...
				part.Name = "UniverseFarmSafetyFloor"
				part.Anchored = true
				...
				part.Transparency = 1
				part.Size = Vector3.new(SAFETY_PLATFORM_SIZE, 1, SAFETY_PLATFORM_SIZE)
				part.Parent = Workspace
				safetyPlatform = part
			end

			local floorY = focus.Y - safetyGap
			safetyPlatform.Size = Vector3.new(SAFETY_PLATFORM_SIZE, 1, SAFETY_PLATFORM_SIZE)
			safetyPlatform.CFrame = CFrame.new(focus.X, floorY, focus.Z)
			return floorY
		end

		local function stabilizeCrystal(inst, anchor, floorY)
			if not safetyEnabled or not inst or not inst.Parent or not floorY then
				return
			end

			pcall(function()
				local velocity = inst.AssemblyLinearVelocity
				if velocity.Y < -2 then
					inst.AssemblyLinearVelocity = Vector3.new(velocity.X, 0, velocity.Z)
				end

				local halfHeight = math.max(0.5, inst.Size.Y * 0.5)
				local minimumY = floorY + halfHeight + SAFETY_CRYSTAL_MARGIN

				if inst.Position.Y < minimumY then
					local rotation = inst.CFrame - inst.Position
					inst.CFrame = CFrame.new(anchor.X, minimumY, anchor.Z) * rotation
		...
		local function guardFarmPosition(root, focus, aim, lift)
			local floorY = safetyFloorAt(focus)
			if not floorY or not root then
				return floorY
			end

			local fallingFast = root.AssemblyLinearVelocity.Y < -70
			local belowFloor = root.Position.Y < floorY + SAFETY_RECOVER_MARGIN

			if fallingFast or belowFloor then
				local recover = focus + Vector3.new(0, lift, 0)
				local frame = CFrame.new(recover)

				if typeof(aim) == "Vector3" and (aim - recover).Magnitude > 0.1 then
					frame = CFrame.lookAt(recover, aim)
				end

				applyPivot(frame)
				Move.glide(frame, aim)
			end
		...
		local function digOffsets(radius)
			if radius <= 0 then
				return { Vector3.zero }
			end
		...
		local function swing(spot, focusedCrystal)
			...
			-- Never use the wide/deep terrain pattern on a priority crystal. One
			-- precise hit per swing avoids removing the support directly below it.
			local offsets = focusedCrystal and { Vector3.zero } or digOffsets(digRadius)
			local depth = focusedCrystal and 1 or math.max(1, math.floor(digDepth + 0.5))
			local total = #offsets * depth
			local requests = focusedCrystal and 1 or math.min(total, DIG_MAX_REQUESTS)

			return pcall(function()
				-- Cycle through the complete width/depth pattern over multiple ticks.
				-- This makes a wider tunnel without firing every point at once.
				for _ = 1, requests do
					digPatternCursor = digPatternCursor % total + 1
					local index = digPatternCursor - 1
					local offsetIndex = index % #offsets + 1
					local layer = math.floor(index / #offsets)
					local point = aim + offsets[offsetIndex] - Vector3.new(0, layer * DIG_SINK, 0)
					event:FireServer(name, point)
				end
			end)
		end
		...
		local function findLoot(free, origin)
			...
			local blocked = false
			local seen = {}
			local seekAcrossLoadedMap = valueFilter and minValue > 0
			...
			local function consider(inst)
				...
				if not seekAcrossLoadedMap and distance > COLLECT_RANGE then
					return
				end
			...
			eachContainer(function(container)
				-- Workspace can be very large, so only recurse inside the dedicated
				-- crystal containers. Registry tracking covers crystals discovered
				-- elsewhere while Auto Farm is active.
				local entries = container == Workspace and container:GetChildren() or container:GetDescendants()
		...
		local function stop()
			...
			Move.setFly(toggleValue("Fly"))
			Move.setNoclip(toggleValue("Noclip"))
			Move.setSafeFall(toggleValue("NoFallDamage"))
			Mountain.setAutoGrab(toggleValue("AutoRunePickup"))
		end
		...
		local function step(deltaTime)
			...
			-- Min Value is a priority target for Money Auto Farm, not only an ESP filter.
			-- As soon as a qualifying crystal is visible in the loaded map, abandon the
			-- current surface column and move directly to that crystal.
			if valueFilter and minValue > 0 and not loot and now - lootClock >= COLLECT_GAP then
				lootClock = now
				loot, lootBlocked = findLoot(backpackFree(), root.Position)

				if loot then
					lootSafetySpot = loot.Position
					loaded = true
					scanIndex = #SCAN_SPOTS + 1
					target = nil
					columnY = nil
					columnDry = 0
					columnSwings = 0
			...
			if not loaded and not loot then
				if scanIndex == 0 then
					scanIndex = 1
				...
				if scanIndex <= #SCAN_SPOTS then
					local scanFrame = SCAN_SPOTS[scanIndex]
					holdAt(scanFrame)
					guardFarmPosition(root, scanFrame.Position, scanFrame.Position, DIG_LIFT)
			...
			local swingNeed = math.max(DIG_MIN_INTERVAL, Farm.swingGap(heldPick) / math.max(1, digRate))
			local canSwing = swingClock >= swingNeed
			local free = backpackFree()

			if loot and (not loot.Parent or getAttr(loot, "Collected") == true or not meetsFilter(loot)) then
				loot = nil
				lootSafetySpot = nil
				lootHp = nil
				lootMax = nil
			...
			if loot then
				...
				local anchor = lootSafetySpot or spot

				holdAt(CFrame.new(spot + Vector3.new(0, COLLECT_LIFT, 0), spot), spot)
				local floorY = guardFarmPosition(root, anchor, spot, COLLECT_LIFT)
				stabilizeCrystal(loot, anchor, floorY)

				requestStream(spot)

				-- Only dig while the crystal itself still has HP. Once it is ready to
				-- collect, stop digging so the floor beneath it remains intact.
				if canSwing and lootHp and lootHp > 0 then
				...
				local targetName = crystalName(loot)
				local targetValue = formatShort(crystalValue(loot), "$")
				...
				if lootHp and lootHp > 0 then
					...
					statusText = string.format("Priority %s %s  %d%%", targetName, targetValue, math.floor(ratio * 100))
				else
					statusText = string.format("Collecting %s %s", targetName, targetValue)
				end
			...
			guardFarmPosition(root, target, target, DIG_LIFT)
		...
		MoneyBox:AddSlider("MoneyDigSpeed", {
			Text = "Dig Speed",
			Default = 2.5,
			Min = 1,
			Max = 5,
			...
			Suffix = "x",
			Callback = function(value)
				digRate = math.clamp(tonumber(value) or 2.5, 1, 5)
			end,
		})

		MoneyBox:AddSlider("MoneyDigWidth", {
			Text = "Dig Width",
			Default = 0,
			Min = 0,
			Max = 6,
			...
			Suffix = " studs",
			Callback = function(value)
				digRadius = math.clamp(tonumber(value) or 0, 0, 6)
				digPatternCursor = 0
			end,
		})

		MoneyBox:AddSlider("MoneyDigDepth", {
			Text = "Dig Depth",
			Default = DIG_BURST,
			Min = 1,
			Max = 12,
			...
			Compact = false,
			Callback = function(value)
				digDepth = math.clamp(math.floor((tonumber(value) or DIG_BURST) + 0.5), 1, 12)
				digPatternCursor = 0
			end,
		})

		MoneyBox:AddLabel("Width cycles through a radial pattern; very high speed may be limited by the server.", true)

		MoneyBox:AddDivider()

		MoneyBox:AddToggle("MoneySafetyPlatform", {
		...
		MoneyBox:AddSlider("MoneySafetyGap", {
			Text = "Safety Space",
			Default = SAFETY_DEFAULT_GAP,
			Min = 4,
			Max = 18,
			Rounding = 0,
			Compact = false,
			Suffix = " studs",
			Callback = function(value)
				safetyGap = math.clamp(tonumber(value) or SAFETY_DEFAULT_GAP, 4, 18)
			end,
		})

		MoneyBox:AddLabel("Priority crystals use precise hits only. The invisible floor and emergency recovery prevent following a falling crystal into the hole.", true)
...
Library:OnUnload(function()
	...
	-- Stop farming first so temporary platforms and movement state are restored
	-- before the movement and mountain modules disconnect permanently.
